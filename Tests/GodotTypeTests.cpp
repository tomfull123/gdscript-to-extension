...
TEST_F(TranspileTest, GodotImageAndImageFormat)
{
	std::string input = R"(
		func doStuff() -> void:
			var with_alpha := false
			var format := Image.FORMAT_RGBA8 if with_alpha else Image.FORMAT_RGB8
			var image := Image.create(0, 0, false, format)
	)";
